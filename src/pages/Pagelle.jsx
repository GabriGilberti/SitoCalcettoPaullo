  ...
  return (
    <div>
      {items.map((player, i) => (
        ...
        >
          <span style={{ color: color, fontWeight: 900, fontSize: 16, width: 24, textAlign: "center" }}>
            {i + 1}
          </span>
          <div style={{
...
export default function Pagelle() {
  ...
  const rated = buildRanking(players, data)
  setResults(rated)
  }
  ...
  async function handleSave() {
    setSaving(true)
    const rows = [
      ...rankingA.map((p, i) => ({ match_id: lastMatch.id, candidate_id: p.id, position: i + 1, wins: 0 })),
      ...rankingB.map((p, i) => ({ match_id: lastMatch.id, candidate_id: p.id, position: i + 1, wins: 0 })),
    ]
            ...
            <div key={r.name} style={{
              ...
              <div style={{ flex: 1 }}>
                ...
                <div style={{ color: C.muted, fontSize: 11, marginTop: 2 }}>
                  posizione media {r.avgPos.toFixed(1)} · {r.team === "A" ? lastMatch.team_a_name : lastMatch.team_b_name}
                </div>
              </div>
              <div style={{
